...
<%
    ...
    ResultSet rs=stmt.executeQuery("select  COUNT(*) from books_information WHERE bookname LIKE '%"+value+"%';");
    while (rs.next()) {
        num = rs.getInt(1);
    }
    ...
    if (pp == null) {
        response.sendRedirect("search.jsp?page=1&book="+value);
    } else {
        page_number = present_page;
        morph = page_number;
        int start = ((morph - 1) * per_page);
        ResultSet rs2 = stmt
                .executeQuery("select * from books_information WHERE bookname LIKE '%"+value+"%'LIMIT "
                        + start + "," + per_page);
%>
<html>
<head>
...
<div class="header1">
    ...
    <div class="input-append" >
        <input type="text" name = "book" class="span2 search-query" style="width:400px;height:30px;background-color:#00987b;border-color:#00987b;color:white;" placeholder="search by bookname">
...
<div class="content">
    ...
    <div class="maincontent">
       <div class="heading">
            ...
            <img alt="" src="images/book-32.png" style="padding-right: 10px;">Search Results
            </p>
        </div>
        <div class="newitem"
        ...
        <div class="navig" style="clear: both; margin-left:700px;padding-top: 50px;">
                ...
                %>
                <a href="search.jsp?page=<%=counter%>&book=<%=value %>" class="pagination"><%=counter%></a>
                <%
                    }
                conn.close();
                stmt.close();
